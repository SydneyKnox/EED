...
int DrawGLScene(Surface *surf)
{
	...
	glBegin(GL_QUADS);
        ...
        glTexCoord2f((GLfloat)pixelWidth/(GLfloat)surf->SurfaceWidth(), 0.0f); glVertex3f( (GLfloat)surf->Width()/3,  0, 0);
		glTexCoord2f((GLfloat)pixelWidth/(GLfloat)surf->SurfaceWidth(), (GLfloat)pixelHeight/(GLfloat)surf->SurfaceHeight()); glVertex3f( (GLfloat)surf->Width()/3,(GLfloat)surf->Height()/3, 0);
		glTexCoord2f(0.0f, (GLfloat)pixelHeight/(GLfloat)surf->SurfaceHeight()); glVertex3f( 0, (GLfloat) surf->Height()/3, 0);
	glEnd();
    glDisable(GL_TEXTURE_2D);

    glBegin(GL_LINE_LOOP);
    glVertex3f( 0,    0, 0);
    glVertex3f( (GLfloat)surf->Width()/3,  0, 0);
    glVertex3f( (GLfloat)surf->Width()/3,(GLfloat)surf->Height()/3, 0);
    glVertex3f( 0, (GLfloat) surf->Height()/3, 0);
    glEnd();
...
void StartScene()
{
    ...
    // Calculate The Aspect Ratio Of The Window
    gluPerspective(45.0f,(GLfloat)gWindowWidth/(GLfloat)gWindowHeight,0.1f,100.0f);
